...
bool ObjectSelection::FilterObjects(LPRDATA rdPtr, short Oi, bool negate,
                                    bool (*filterFunction)(LPRDATA, LPRO)) {
  if (Oi & 0x8000)
    return FilterQualifierObjects(rdPtr, Oi & 0x7FFF, negate, filterFunction) ^
           negate;
  else
    return FilterNonQualifierObjects(rdPtr, Oi, negate, filterFunction) ^
           negate;
}

// Filter qualifier objects
bool
ObjectSelection::FilterQualifierObjects(LPRDATA rdPtr, short Oi, bool negate,
                                        bool (*filterFunction)(LPRDATA, LPRO)) {
  ...
  while (CurrentQualToOi->qoiOiList >= 0) {
    ...
    hasSelected |= FilterNonQualifierObjects(rdPtr, CurrentOi->oilOi, negate,
                                             filterFunction);
...
bool ObjectSelection::FilterNonQualifierObjects(LPRDATA rdPtr, short Oi,
                                                bool negate,
                                                bool (*filterFunction)(LPRDATA,
                                                                       LPRO)) {
  LPOIL pObjectInfo = GetOILFromOI(Oi);
  bool hasSelected = false;
