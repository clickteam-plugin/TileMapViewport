...
bool checkRectangleOverlap(LPRDATA rdPtr, Layer &layer, Tileset &tileset,
  ...
  if (!rdPtr->outsideColl) {
    if (rect.x2 < rdPtr->rHo.hoX - rdPtr->collMargin.left ||
        ...
        rect.x1 >
            rdPtr->rHo.hoX + rdPtr->rHo.hoImgWidth + rdPtr->collMargin.right ||
        rect.y1 >
            rdPtr->rHo.hoY + rdPtr->rHo.hoImgHeight + rdPtr->collMargin.bottom)
      return false;
  }
  ...
  while (rectCount--) {
    ...
    if (rectCount > RECT_MAX)
      MessageBox(
          0, "Tile Map has a problem. Please post this in the forum thread.",
...
bool checkPixelSolid(LPRDATA rdPtr, Layer &layer, Tileset &tileset, int pixelX,
  ...
  return surface->GetPixelFast(tilesetX, tilesetY) !=
         surface->GetTransparentColor();
}
