...
short WINAPI DLLExport DisplayRunObject(LPRDATA rdPtr) {
  ...
  if (minLayer <= maxLayer && maxLayer < layerCount) {
    for (unsigned i = minLayer; i <= maxLayer; ++i) {
      ...
      int borderX =
          1 + (rdPtr->tileCallback.use ? rdPtr->tileCallback.borderX : 0);
      int borderY =
          1 + (rdPtr->tileCallback.use ? rdPtr->tileCallback.borderY : 0);
      ...
      while (drawX >=
             viewportWidth + pixelBorderX - (x2 - x1) * renderTileWidth)
        x2--;
      while (drawY >=
             viewportHeight + pixelBorderY - (y2 - y1) * renderTileWidth)
        y2--;
      ...
      if (settings.wrapX) {
        ...
        while (drawX <
               viewportWidth - renderTileWidth * (x2 - x1 - borderX + 1))
          x2++;
      ...
      if (settings.wrapY) {
        ...
        while (drawY <
               viewportWidth - renderTileWidth * (y2 - y1 - borderY + 1))
          y2++;
          ...
          for (int unwrappedX = x1; unwrappedX < x2; ++unwrappedX) {
            ...
            do {
              ...
              if (tileSettings.transform) {
                ...
                float blitX =
                    drawX + (screenX - drawX + offsetX + tileCenter.x) * zoom;
                float blitY =
                    drawY + (screenY - drawY + offsetY + tileCenter.y) * zoom;
              ...
              if (!clip) {
                tileSurf->Blit(*target, screenX + offsetX, screenY + offsetY,
                               (tile.x) * tileWidth, tile.y * tileHeight,
                               tileWidth, tileHeight, BMODE_TRANSP, blitOp,
                               blitParam, blitFlags);
              }
              ...
              // the viewport...
                ...
                if (dX < viewportWidth && dY < viewportHeight && sW > 0 &&
                    sH > 0) {
