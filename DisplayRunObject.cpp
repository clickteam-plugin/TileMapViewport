...
// Clips a row of tiles rendered onto a viewport to render only within the viewport boundaries
					...
					for (int unwrappedX = x1; unwrappedX < x2; ++unwrappedX)
					{
						...
						do
						{
							...
							if (tileSettings.transform)
							{
								...
								tileSurf->BlitEx(*target, blitX, blitY, scaleX, scaleY, tile.x*tileWidth, tile.y*tileHeight,
                                    tileWidth, tileHeight, &tileCenter, angle, BMODE_TRANSP, blitOp, blitParam, blitFlags);
								break;
							}
#endif
							...
							if (!clip)
							{
                                tileSurf->Blit(*target, screenX+offsetX, screenY+offsetY, (tile.x) * tileWidth, tile.y * tileHeight,
                                    tileWidth, tileHeight, BMODE_TRANSP, blitOp, blitParam, blitFlags);
							}
							...
							else
							{
								...
								if (dX < viewportWidth && dY < viewportHeight && sW > 0 && sH > 0)
								{
									...
									tileSurf->Blit(*target, dX, dY, sX, sY, sW, sH, BMODE_TRANSP, blitOp, blitParam, blitFlags);
								}
							}
						}
						while (0);
